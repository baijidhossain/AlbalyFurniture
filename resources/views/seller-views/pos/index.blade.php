                                    ...
                                    <div class="input-group">
                                        <select name="category" id="category" class="form-control js-select2-custom mx-1"
                                            ...
                                            @foreach ($categories as $item)
                                                <option
                                                    value="{{$item->id}}" {{$category==$item->id?'selected':''}}>{{$item->defaultName}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                                    <form class="">
                                        ...
                                        <div class="input-group-overlay input-group-merge input-group-custom">
                                            ...
                                            <diV class="card search-card w-4 position-absolute z-index-1 w-100">
                ...
                <div class="col-lg-5">
                    <div class="card billing-section-wrap">
                        ...
                        <div class="card-body">
                            ...
                            <div id="cart-summary">
                                @include('seller-views.pos._cart-summary')
                            </div>
...
<script>
    ...
    function getVariantPrice(type = null) {
        if ($('#add-to-cart-form input[name=quantity]').val() > 0 && checkAddToCartValidity()) {
            ...
            $.ajax({
                type: "POST",
                url: '{{ route('seller.pos.variant_price') }}'+(type ? '?type='+type : ''),
                ...
                success: function (data) {
                    ...
                    if (data.data == 1) {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: "{{ translate('Cart') }}",
                            text: '{{ translate("sorry_product_out_of_stock")}}'
                        });
                        return false;
                    }
    ...
    function edit_customer_modal()
    {
        ...
        $.post({
            url: "{{route('seller.pos.customer-edit')}}",
            data: {
                _token: '{{csrf_token()}}',
    ...
    function view_all_hold_orders(action = null)
    {
        ...
        $.post({
            url: "{{route('seller.pos.view-all-hold-orders')}}",
            data: {
                _token: '{{csrf_token()}}',
    ...
    function cancel_customer_order(cart_id)
    {
        ...
        $.post({
            url: "{{route('seller.pos.cancel-customer-order')}}",
            data: {
                _token: '{{csrf_token()}}',
                cart_id: cart_id,
            },
