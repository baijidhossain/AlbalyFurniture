                            ...
                            @foreach($coupons as $k=>$coupon)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <span
                                            class="ml-1">{{translate('used')}}: <strong>{{ $coupon['order_count'] }}</strong></span>
                                    </td>
                                    ...
                                    <td>
                                        @if($coupon->added_by=='seller' || ($coupon->added_by=='admin' && $coupon->coupon_bearer=='seller' && $coupon->seller_id==auth('seller')->id()))
                                            <form action="{{route('seller.coupon.status',[$coupon['id'],$coupon->status?0:1])}}" method="GET" id="coupon_status{{$coupon['id']}}_form" class="coupon_status_form">
                                                <label class="switcher">
                                                    <input type="checkbox" class="switcher_input" id="coupon_status{{$coupon['id']}}" name="status" value="1" {{ $coupon->status == 1 ? 'checked':'' }} onclick="toogleStatusModal(event,'coupon_status{{$coupon['id']}}','coupon-status-on.png','coupon-status-off.png','{{translate('Want_to_Turn_ON_Coupon_Status')}}','{{translate('Want_to_Turn_OFF_Coupon_Status')}}',`<p>{{translate('if_enabled_this_coupon_will_be_available_on_the_website_and_customer_app')}}</p>`,`<p>{{translate('if_disabled_this_coupon_will_be_hidden_from_the_website_and_customer_app')}}</p>`)">
