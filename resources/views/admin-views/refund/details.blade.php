                ...
                <div class="card-body">
                    <div class="row gy-2">
                        ...
                        <div class="col-sm-8 col-md-4 col-lg-6">
                            ...
                            <div class="mb-1">{{translate('QTY')}} : {{$refund->order_details->qty}}</div>
                            <div class="mb-1">{{translate('price')}} : {{\App\CPU\Helpers::currency_converter($refund->order_details->price)}}</div>
                    ...
                    <div class="d-flex flex-wrap flex-column flex-md-row gap-10 justify-content-between">
                        <span class="title-color">{{translate('subtotal')}} : {{\App\CPU\Helpers::currency_converter($subtotal)}}</span>
                        <span class="title-color">{{translate('coupon_discount')}} : {{\App\CPU\Helpers::currency_converter($coupon_discount)}}</span>
                        <span class="title-color">{{translate('total_refund_amount')}} : {{\App\CPU\Helpers::currency_converter($refund_amount)}}</span>
                    </div>
                ...
                <div class="card-body">
                    <div class="row gy-2">
                        <div class="col-sm-6 col-md-4 d-flex flex-column gap-10">
                            ...
                            @else
                                <div>{{translate('seller_not_found')}} </div>
                            @endif
                        </div>

                        <div class="col-sm-6 col-md-4 d-flex flex-column gap-10">
                            <h5>{{translate('deliveryman_info')}} : </h5>
                            <div>{{translate('deliveryman_name')}} : {{$order->delivery_man!=null?$order->delivery_man->f_name . ' ' .$order->delivery_man->l_name:translate('not_assigned')}}</div>
                            <div>{{translate('deliveryman_email')}} : {{$order->delivery_man!=null?$order->delivery_man->email :translate('not_found')}}</div>
                            <div>{{translate('deliveryman_phone')}} : {{$order->delivery_man!=null?$order->delivery_man->phone :translate('not_found')}}</div>
                        </div>

                        <div class="col-sm-6 col-md-4 d-flex flex-column gap-10">
                            <div>{{translate('payment_method')}} : {{translate($order->payment_method)}}</div>
                            <div class="d-flex flex-wrap gap-2">{{translate('order_details')}} : <a class="btn btn--primary btn-sm" href="{{route('admin.orders.details',['id'=>$order->id])}}">{{translate('click_here')}}</a></div>
...
function refund_status_change(val)
{
    ...
    {
        Swal.fire({
            title: '{{ translate("are_you_sure") }}! {{ translate("after_refunded_you_can_not_change_it") }}!!',
            type: 'warning',
        });
    ...
    <script>
        function refund_stats_update(val) {
            ...
            text: '{{translate("you_want_to_change_refund_status")}}!',
            type: 'warning',
            ...
            $.post({
                ...
                success: function (data) {
                    //console.log(data);
                    if(data === 'success'){
                        toastr.success('{{translate("refund_status_updated_successfully")}}!', {
                            CloseButton: true,
                            ProgressBar: true
                        });
                    }else{
                        toastr.warning('{{translate("it_is_already_refunded_you_can_not_change_it")}}!', {
                            CloseButton: true,
                            ProgressBar: true
                        });
                    }
