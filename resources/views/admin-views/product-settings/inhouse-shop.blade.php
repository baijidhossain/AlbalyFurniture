...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="card mb-3">
            <div class="card-body">
                <form action="{{route('admin.product-settings.inhouse-shop-temporary-close')}}" method="post" id="temporary_close_form">
                        @csrf
                    <div class="border rounded border-color-c1 px-4 py-3 d-flex justify-content-between mb-1">
                        ...
                        <div class="position-relative">
                            <label class="switcher" onclick="toogleStatusModal(event,'temporary_close','store-temporary-close-off.png','store-temporary-close-on.png','{{translate('want_to_Turn_OFF_the_Temporary_Close_option')}}','{{translate('want_to_Turn_ON_the_Temporary_Close_option')}}',`<p>{{translate('if_disabled_this_feature_will_be_hidden_from_the_system')}}</p>`,`<p>{{translate('if_enabled_admin_can_temporarily_pause_his_shop_activities')}}</p>`)">
                    ...
                    <p>*{{translate('by_turning_on_the')}} "{{translate('temporary_Close')}}”
                        {{translate('button_admin_can_pause_his_shop_activities_and_his_shop_will_be_shown_as')}} "{{translate('temporary_Close')}}” {{translate('in_the_system')}}. {{translate('Customers_will_not_be_able_to_order_or_purchase_from_his_shop')}}</p>
                </form>
            </div>
        </div>
            ...
            <div class="card-body">
                <div class="d-flex flex-wrap gap-3 justify-content-between mb-4">
                    <div class="d-flex flex-column gap-1">
                        ...
                        @php($admin = \App\Model\Admin::find(1))
                        <p class="mb-0">{{translate('created_at')}} {{ date('d M, Y', strtotime($admin['updated_at']) )}}</p>
                    </div>
                    <div class="d-flex flex-wrap gap-3">
                        <button class="btn btn-outline--primary" data-bs-toggle="modal" data-bs-target="#vacation_mode_modal">
                            {{translate('go_to_Vacation_Mode')}}
                        </button>
                        <a href="{{ route('admin.product-settings.inhouse-shop') }}?action=edit" class="btn btn--primary d-flex gap-2 align-items-center"><i class="tio-edit"></i>{{translate('edit_shop')}}</a>
                    </div>
                </div>
                ...
                @if ($minimum_order_amount_status || $free_delivery_status)
                <div class="mt-5">
                    <form action="{{ route('admin.product-settings.inhouse-shop') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="bg-white rounded border">
                            ...
                            <div class="card-body">
                                ...
                                <div class="row">
                                    ...
                                    @if($free_delivery_status)
                                    @php($free_delivery_over_amount=\App\CPU\Helpers::get_business_settings('free_delivery_over_amount'))
                                    <div class="col-lg-4 col-mg-6">
                                        <div class="form-group">
                                            <label class="title-color d-flex" for="free_delivery_over_amount">{{ translate('free_Delivery_Over_Amount') }} ({{ \App\CPU\BackEndHelper::currency_symbol() }})</label>
                                            <input type="number" min="0" class="form-control" name="free_delivery_over_amount" id="free_delivery_over_amount" placeholder="{{translate('ex')}}: 10" value="{{\App\CPU\Convert::default($free_delivery_over_amount) }}">
                                        </div>
                                    </div>
                                    @endif
                                </div>
                        ...
                        <div class="d-flex justify-content-end mt-30">
                            <button type="submit" class="btn btn--primary px-4">{{translate('save_informtion')}}</button>
                        </div>
                    </form>
                </div>
                @endif
...
@push('script')
    <script>
        ...
        $('#temporary_close_form').on('submit', function(e){
            e.preventDefault();
            ...
            $.ajax({
                ...
                success: function (data) {
                    toastr.success('{{translate("temporary_close_updated_successfully")}}!');
                    location.reload();
                }
            });
        });
        ...
        // === Strat Js For Offter Banner Theme Fashion
        ...
        // === End Js For Offter Banner Theme Fashion
