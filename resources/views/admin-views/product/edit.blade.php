...
@section('content')
    ...
    <div class="content container-fluid">
        ...
        <!-- Form -->
        <form class="product-form text-start" action="{{route('admin.product.update',$product->id)}}" method="post" enctype="multipart/form-data" id="product_form">
            @csrf
            ...
            <div class="d-flex justify-content-end mt-3">
                @if($product->request_status == 2)
                    <button type="button" onclick="check()" class="btn btn--primary px-5">{{translate('update_&_Publish')}}</button>
                @else
...
@push('script_2')
    ...
    <script>
        ...
        var imageCount = {{15-count(json_decode($product->images))}};
