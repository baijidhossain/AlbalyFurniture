...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('admin.customer.wallet.bonus-setup-store') }}" id="form-data" method="post">
                    @csrf
                    <div class="row gx-2">
                        <div class="col-sm-6 col-md-6">
                            <div class="form-group">
                                <label for="bonus_title" class="title-color text-capitalize d-flex">{{translate('bonus_title')}}</label>
                                <input type="text" name="title" class="form-control" id="bonus_title" placeholder="{{translate('Ex: EID_Dhamaka')}}" required>
                            </div>
                        </div>
                        <div class="col-sm-6 col-md-6">
                            <div class="form-group">
                                <label for="short_desc" class="title-color text-capitalize d-flex">{{translate('short_description')}}</label>
                                <input type="text" name="description" class="form-control" id="short_desc" placeholder="{{translate('Ex: EID_Dhamaka')}}">
                            </div>
                        </div>
                        ...
                        <div class="col-sm-6 col-md-6 col-lg-4" id="bonus_amount_area">
                            <div class="form-group">
                                <label for="bonus_amount" class="title-color text-capitalize d-flex">{{translate('bonus_amount')}} (<span id="bonus_title_percent">%</span>)</label>
                                <input type="number" name="bonus_amount" min="0" class="form-control" value="0" id="bonus_amount" placeholder="{{translate('Ex: 5')}}">
                            </div>
                        </div>
                        <div class="col-sm-6 col-md-6 col-lg-4">
                            <div class="form-group">
                                ...
                                <input type="number" name="min_add_money_amount" min="0" class="form-control" id="min_add_money_amount" value="0" placeholder="{{translate('Ex: 100')}}" required>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-6 col-md-6 col-lg-4" id="max_bonus_amount_area">
                            <div class="form-group">
                                ...
                                <input type="number" min="0" name="max_bonus_amount" value="0" class="form-control" id="max_bonus_amount" placeholder="{{translate('Ex: 5000')}}">
                            </div>
                        </div>
                        <div class="col-sm-6 col-md-6 col-lg-4">
                            <div class="form-group">
                                <label for="start_date_time" class="title-color text-capitalize d-flex">{{translate('start_date')}}</label>
                                <input type="date" name="start_date_time" id="start_date_time" class="form-control" required>
                            </div>
                        </div>
                        <div class="col-sm-6 col-md-6 col-lg-4">
                            <div class="form-group">
                        ...
                        <form action="{{ url()->current() }}" method="GET">
                            <div class="input-group input-group-merge input-group-custom">
                                ...
                                <input id="datatableSearch_" type="search" name="search" class="form-control"
                                        placeholder="{{translate('search_by_bonus_title')}}" value="{{ request('search') }}"
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex flex-column gap-1">
                                    <div>{{translate('minimum_add_amount')}} - {{\App\CPU\Helpers::currency_converter($item->min_add_money_amount) }}</div>
                                    @if ($item->bonus_type != "fixed")
                                        <div>{{translate('maximum_bonus')}} - {{\App\CPU\Helpers::currency_converter($item->max_bonus_amount) }}</div>
                                    @endif
                                </div>
                            </td>
                            <td class="text-center">{{ $item->bonus_type == 'percentage' ? $item->bonus_amount.'%' : \App\CPU\Helpers::currency_converter($item->bonus_amount) }}</td>
                            ...
                            <td>
                                <label class="switcher">
                                    <input type="checkbox" class="switcher_input status" data-id="{{ $item->id }}" {{ ($item->is_active == 1 ? 'checked':'') }}>
        ...
        $(document).on('change', '.switcher_input', function () {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "{{route('admin.customer.wallet.bonus-setup-status')}}",
                        method: 'POST',
                        data: {
                            id: id,
                            status: status
